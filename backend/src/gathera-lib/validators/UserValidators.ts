 ...
 */
export const validateFeedback = (feedback: string) => {
    return validateNotNullUndefined(feedback) && validateString(feedback) && feedback.length <= 200;
};

/**
 * Validates a feedback object.
 * @param ratings
 * @returns true if the feedback object is valid, false otherwise
 */
export const validateRatings = (ratings: { idea: number; easeOfUse: number }) => {
    if (!ratings) return false;
    if (ratings.idea === undefined || ratings.easeOfUse === undefined) return false;
    if (ratings.idea < -1 || ratings.idea > 5 || ratings.easeOfUse < -1 || ratings.easeOfUse > 5) return false;

    return validateNotNullUndefined(ratings) && validateNotNullUndefined(ratings.idea) && validateNotNullUndefined(ratings.easeOfUse);
};
